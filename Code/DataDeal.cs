...
namespace UAV_Info
{
    class DataDeal
    {
        ...
        //全局数据
        private List<UAV_Node> uav_nodes = new List<UAV_Node>();
        private List<UAV_Link> uav_links = new List<UAV_Link>();
        private List<UAV_Config> uav_configs = new List<UAV_Config>();

        //数据库
        private SQLiteConnection cn = null;             //连接系统表格
        private SQLiteConnection conn = null;           //连接历史飞行数据表
        ...
        //飞行任务字典
        private Dictionary<int, string> LogDictionary = new Dictionary<int, string>();
        ...
        void InitTimer()
        {
            TimerNode.Interval = new TimeSpan(0, 0, 0, 0, 300);
            TimerNode.Tick += new EventHandler(TimerNode_Tick);
            TimerNode.Start();

            TimerLog.Interval = new TimeSpan(0, 0, 5);
        ...
        void DealNode(List<UAV_Node> uAVs)
        {
            ...
            if(uav_nodes.Count != 0)
            {
                foreach(var uno in uav_nodes.ToArray())      //删除新节点中不存在的旧节点
                {
                    ...
                    if (!IsDelete)
                    {
                        ...
                        string droplink = string.Format("DELETE FROM Ulink WHERE Source = {0} OR Target = {1}", uno.id, uno.id);
                        SQLiteCommand comdpl = new SQLiteCommand(droplink, cn);
                        comdpl.ExecuteNonQuery();
                        string dropconf = string.Format("DELETE FROM Uconfig WHERE Id = {0}", uno.id);
                        SQLiteCommand comdpc = new SQLiteCommand(dropconf, cn);
            ...
            foreach(UAV_Node uvn in uAVs)
            {
                ...
                if (IdAlready)          //更新新节点中存在的旧节点
                {
                    ...
                    tmp.latitude = uvn.latitude;
                    tmp.longitude = uvn.longitude;
                    tmp.altitude = uvn.altitude;
                    tmp.yaw = uvn.yaw;
                    tmp.pitch = uvn.pitch;
                    tmp.roll = uvn.roll;
                    //更新数据库
                    ...
                    if(Ins_Log %2 == 0)
                    {
                        if (LogDictionary.ContainsKey(uvn.id))
                        {
        ...
        void CreateLog(UAV_Node unode)//创建新的历史飞行任务
        {
            ...
            string taski = "Task" + unode.id + "M" + dt.Month + "D" + dt.Day + "H" + dt.Hour + "m" + dt.Minute;
            //string ftime = string.Format("{0:yyyy-MM-dd HH:mm:ss}", dt);
            string ftime = string.Format("{0:yyyy-MM-dd HH:mm:ss}", dt);
            LogDictionary.Add(unode.id, taski);
            string sqllog = string.Format("INSERT INTO Tasks(Id, Devname, TaskId, FlyTime) VALUES ({0}, '{1}', '{2}', '{3}')", unode.id,
                dname, taski, ftime);
        ...
        void InitFlyLog()
        {
            ...
            if (!tableTaskExist)
            {           //创建索引表
                commd.CommandText = "create table Tasks(Id INTEGER, Devname TEXT, TaskId TEXT, FlyTime TEXT)";
